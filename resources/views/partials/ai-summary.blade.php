...
@if($ps)
    {{-- Structured format: Problem → Solution → Timeline → Commission → Result --}}
    <div class="ai-summary__title">{{ $ps['title'] }}</div>
    <dl class="ai-summary__dl" style="margin:0;line-height:1.7">
        <dt style="font-weight:700;margin-top:.5rem">{{ $locale === 'ru' ? '⚠️ Проблема' : ($locale === 'en' ? '⚠️ Problem' : '⚠️ Probleem') }}</dt>
        <dd style="margin:0 0 .3rem 0">{{ $ps['problem'] }}</dd>
        <dt style="font-weight:700;margin-top:.5rem">{{ $locale === 'ru' ? '✅ Решение' : ($locale === 'en' ? '✅ Solution' : '✅ Lahendus') }}</dt>
        <dd style="margin:0 0 .3rem 0">{{ $ps['solution'] }}</dd>
        <dt style="font-weight:700;margin-top:.5rem">{{ $locale === 'ru' ? '⏱ Срок' : ($locale === 'en' ? '⏱ Timeline' : '⏱ Ajakava') }}</dt>
        <dd style="margin:0 0 .3rem 0">{{ $ps['timeline'] }}</dd>
        <dt style="font-weight:700;margin-top:.5rem">{{ $locale === 'ru' ? '💰 Комиссия' : ($locale === 'en' ? '💰 Commission' : '💰 Vahendustasu') }}</dt>
        <dd style="margin:0 0 .3rem 0">{{ $ps['commission'] }}</dd>
        <dt style="font-weight:700;margin-top:.5rem">{{ $locale === 'ru' ? '🎯 Результат' : ($locale === 'en' ? '🎯 Result' : '🎯 Tulemus') }}</dt>
        <dd style="margin:0">{{ $ps['result'] }}</dd>
    </dl>
@else
    {{-- Generic fallback --}}
    <div class="ai-summary__title">{{ $titleFallback[$locale] ?? 'Quick Summary' }}</div>
    <p>{{ $genericSummaries[$locale] ?? $genericSummaries['en'] }}</p>
    <ul>
        @foreach (($genericDetails[$locale] ?? $genericDetails['en']) as $item)
            <li>{{ $item }}</li>
        @endforeach
    </ul>
@endif
