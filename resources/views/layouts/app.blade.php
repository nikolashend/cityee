...
<link href="/assets/css/cityee-phase5-6.css?v=5" rel="stylesheet">
@else
...
<link href="/assets/css/cityee-phase5-6.css?v=5" rel="stylesheet" media="print" onload="this.media='all'">
<noscript><link href="/assets/css/cityee-phase5-6.css?v=5" rel="stylesheet"></noscript>
@endif
  ...
  <nav class="nav" aria-label="{{ $locale === 'ru' ? 'Главное меню' : ($locale === 'en' ? 'Main navigation' : 'Peamine navigatsioon') }}">
    <div class="container">
      ...
      @php
        ...
        try { $langEtUrl = \Illuminate\Support\Facades\Route::has($langEtRoute) ? route($langEtRoute) : route('et.home'); } catch (\Throwable $e) { $langEtUrl = route('et.home'); }
        try { $langRuUrl = \Illuminate\Support\Facades\Route::has($langRuRoute) ? route($langRuRoute) : route('ru.home'); } catch (\Throwable $e) { $langRuUrl = route('ru.home'); }
        try { $langEnUrl = \Illuminate\Support\Facades\Route::has($langEnRoute) ? route($langEnRoute) : route('en.home'); } catch (\Throwable $e) { $langEnUrl = route('en.home'); }
      @endphp
      <div class="languages">
...
<script src="/assets/templates/offshors/js/main.js?v=4" defer></script>
